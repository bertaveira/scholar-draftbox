...
Run `npm run data:links` from `scout/` to validate and apply the accepted links and abstracts. This requires no network access or raw cache files. Official ingestion also reapplies this enrichment when these artifacts are present. Build afterwards to update the preview/offline bundle.
