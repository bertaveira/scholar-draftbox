...
- Static similar-paper suggestions in each detail panel and a dedicated Suggestions page with balanced, explainable recommendations. Ranking runs entirely in the browser from precomputed neighbours; suggestions can be dismissed locally.
...
Static similar-paper neighbours and normalized embeddings are generated by the repeatable pipeline in [`scripts/recommendations/`](scripts/recommendations/README.md). It compares a title/topic keyword baseline with two pinned open embedding models, uses content-addressed row caching, validates complete versions before an atomic pointer switch, and includes a CPU fallback. The checked-in artifact uses SPECTER2's scientific-paper proximity adapter. The app loads the compact neighbour graph only when Suggestions or a paper detail is opened: detail panels show five similar papers, while Suggestions fairly interleaves candidates across separate bookmark-interest groups and reports which saved papers contributed. The build precaches the neighbour graph for offline use, but excludes the pipeline-only binary embeddings and row-order file from the browser cache. Free-text semantic search remains intentionally unimplemented because it needs a separate query-embedding path.
...
Bookmarks use `eccv-scout.profile.v1` in browser localStorage. Export a backup before clearing browser data or changing origins. Profiles contain a schema version, conference identifier, and saved paper IDs. Imports reject malformed or incompatible profiles before changing state. Dismissed recommendations use the separate `eccv-scout.recommendation-dismissals.v1` key and are not included in bookmark exports. Storage errors leave in-memory bookmarks and dismissals usable for the current visit.
...
Automated coverage includes data validation, full-source deduplication, multiple appearances per paper, combined search filters, accent matching, Stockholm day boundaries, natural poster ordering, profile merging, recommendation artifact validation/ranking/loading, local dismissals, cross-tab updates, storage failures, malformed refresh fallback, and offline service-worker navigation/cache behavior.
...
The lint command covers authored application, data tooling, and test code. Unmodified supplied component primitives contain existing lint findings and are excluded from that command. Public hosting, free-text semantic search, workshops, and calendar export remain future work.
